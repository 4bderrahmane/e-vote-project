...
interface IElection {
    ...
    event ElectionStarted(address indexed coordinator, uint256 startTime, uint256 endTime);
    ...
    event VoteAdded(bytes32 indexed ciphertextHash, uint256 indexed nullifier, bytes ciphertext);
    ...
    event ElectionEnded(address indexed coordinator, bytes decryptionMaterial);
    ...
    function castVote(bytes calldata ciphertext, uint256 nullifier, uint256[8] calldata proof) external;
