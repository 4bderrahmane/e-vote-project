...
check "nmap: port 22 is open" "pass" \
  "sudo nmap -sS -Pn -p 22 --max-retries 1 --host-timeout 10s $FW 2>/dev/null | grep -q '22/tcp.*open'"
check "nmap: random port 9090 is filtered" "pass" \
  "sudo nmap -sS -Pn -p 9090 --max-retries 1 --host-timeout 10s $FW 2>/dev/null | grep -q '9090/tcp.*filtered'"
