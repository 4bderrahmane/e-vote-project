...
public class VoterRegistrationService
{
    ...
    private static void requireCitizenEligible(Citizen citizen)
    {
        ...
        {
            throw new BusinessConflictException("Citizen is not eligible to register for elections");
        }
    }
    ...
    private static void requireRegistrationOpen(Election election)
    {
        ...
        {
            throw new BusinessConflictException("Election is not accepting voter registrations");
        }
        if (election.getEndTime() != null && !election.getEndTime().isAfter(Instant.now()))
        {
            throw new BusinessConflictException("Election registration window has already elapsed");
        }
    }
    ...
    private static String requireContractAddress(Election election)
    {
        ...
        {
            throw new BusinessConflictException("Election must be deployed before voter registration");
        }
    ...
    private static BigInteger requireGroupId(Election election)
    {
        ...
        {
            throw new BusinessConflictException("Election externalNullifier is required for voter registration");
        }
    ...
    private static BigInteger parseIdentityCommitment(String identityCommitment)
    {
        ...
        {
            throw new RequestValidationException("identityCommitment is required");
        }
        ...
        try
        {
            ...
            if (value.signum() <= 0)
            {
                throw new RequestValidationException("identityCommitment must be a positive decimal integer");
            }
            return value;
        } catch (NumberFormatException ex)
        {
            throw new RequestValidationException("identityCommitment must be a positive decimal integer", ex);
        }
    }
    ...
    )
    {
        if (request == null)
        {
            throw new RequestValidationException("Registration payload is required");
        }
        ...
        {
            throw new BusinessConflictException("identityCommitment is already registered for this election", ex);
        }
        ...
        try
        {
            ...
            if (response == null)
            {
                throw new OperationFailedException("Failed to persist voter commitment");
            }
            return response;
        } catch (Exception ex)
        {
            markCommitmentFailed(prepared.electionPublicId(), prepared.citizenKeycloakId());
            throw new OperationFailedException("Failed to enroll voter commitment on chain", ex);
        }
    }
    ...
    )
    {
        PreparedRegistration prepared = transactionOperations.execute(status ->
        {
            ...
            {
                throw new BusinessConflictException("Citizen already registered a different identityCommitment for this election");
            }
            ...
            voterCommitmentRepository.findByElectionPublicIdAndIdentityCommitment(electionPublicId, normalizedCommitment)
                    .ifPresent(existing ->
                    {
                        ...
                        {
                            throw new BusinessConflictException("identityCommitment is already registered for this election");
                        }
                    });
        ...
        if (prepared == null)
        {
            throw new OperationFailedException("Failed to prepare voter registration");
        }
        return prepared;
    }
    ...
    ) throws Exception
    {
        ...
        if (!electionClient.hasMember(contractAddress, groupId, commitmentValue))
        {
            ...
            {
                throw new OperationFailedException("Blockchain transaction hash is required when adding voter commitment");
            }
        }
