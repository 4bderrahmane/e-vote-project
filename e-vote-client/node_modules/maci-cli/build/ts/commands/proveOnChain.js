 ...
 */
const proveOnChain = async ({ pollId, proofDir, maciAddress, signer, tallyFile, quiet = true, }) => {
    ...
    if (tallyFile) {
        const tallyData = await fs_1.default.promises.readFile(tallyFile).then((res) => JSON.parse(res.toString()));
        const tallyResults = tallyData.results.tally.map((t) => BigInt(t));
        const tallyResultProofs = tallyData.results.tally.map((_, index) => (0, maci_crypto_1.genTreeProof)(index, tallyResults, Number(treeDepths.voteOptionTreeDepth)));
        await tallyContract
            .addTallyResults({
